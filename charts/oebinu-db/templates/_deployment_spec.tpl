...
{{- end }}
  selector:
    matchLabels:
      ...
      app.kubernetes.io/instance: {{ .Release.Name }}
      {{- if .isCanary }}
      cibcwork.io/deployment-type: canary
      {{- else }}
      cibcwork.io/deployment-type: main
      {{- end }}
  template:
    metadata:
      labels:
...
{{ toYaml .Values.lifecycleHooks.preStop | indent 14 }}
            ...
            # See https://blog.cibcwork.io/delaying-shutdown-to-wait-for-pod-deletion-propagation-445f779a8304
            preStop:
              exec:
                command:
                  - sleep
